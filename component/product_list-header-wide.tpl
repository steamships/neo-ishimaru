...
<{if $tpl_name == "product_list"}>
	...
	<{/if}>
		<{if $bid_name != "" && $smarty.get.cbid == $category[num].bid}>
			...
			<img class="p-category-main-image-img d-block d-lg-none" src="https://img07.shop-pro.jp/PA01437/442/etc/neo-category-img-<{$smarty.get.cbid}>-sp.jpg" alt="<{$category[num].name}>カテゴリー画像">
	...
	<{/if}>
		...
		<img class="p-category-main-image-img d-block d-lg-none" src="https://img07.shop-pro.jp/PA01437/442/etc/neo-category-img-<{$smarty.get.gid}>-sp.jpg" alt="<{$group[num].name}>カテゴリー画像">
		<!-- <{$group[num].name}>の説明文 -->
		...
		<p class="p-category-main-image-text">自然豊かな九州で育った黒毛和牛です。味はもちろん、国産で安心・安全さも魅力の一つです！<br>脂が乗りすぎてたくさん食べられないという方に九州産黒毛和牛はおすすめです。</p>
		<{elseif $smarty.get.gid == "2483504"}>
		<p class="p-category-magidin-image-text">ハンバーグやローストビーフなどの加工食品をはじめ、希少部位や期間限定のお肉などを随時掲載していきます。まだ、あなたの知らないお肉に出会えるかも…</p>
		<{/if}>
		<{/if}>
	<{if $smarty.section.num.last }>
	</div>
